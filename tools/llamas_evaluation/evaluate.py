...
import tools.llamas_utils as utils

with open('configs.yaml', 'r') as f:
    configs = yaml.load(f, Loader=yaml.Loader)

LLAMAS_IMG_RES = configs['LLAMAS']['SIZES'][1]
...
def load_labels(label_dir):
    ...
    """
    label_paths = utils.get_files_from_folder(label_dir, '.json')
    annos = [[add_ys(xs) for xs in utils.get_horizontal_values_for_four_lanes(label_path) if
              (np.array(xs) >= 0).sum() > 1]  # lanes annotated with a single point are ignored
             for label_path in label_paths]
    label_paths = [
        utils.get_label_base(p) for p in label_paths
    ]
