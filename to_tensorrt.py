...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        trt_outputs = inference_trt(engine_binary=engine, input_tensor=in_t)
        onnx_outputs = inference_onnx(dummy, args.onnx_path)
        diff = 0.0
        avg = 0.0
        for (_, temp_trt), temp_onnx in zip(trt_outputs.items(), onnx_outputs):
